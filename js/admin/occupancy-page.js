...
(function occupancyPage(){
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      .occupancy-v2-head{display:block;margin-bottom:2px}
      ...
      .occupancy-v2-copy h1{margin:3px 0 4px;color:var(--text);font-size:1.45rem;line-height:1.15}
      .occupancy-v2-copy>p{margin:0;color:var(--muted);font-size:.7rem}

      .occupancy-v2-toolbar{display:grid;grid-template-columns:repeat(3,minmax(0,1fr)) minmax(210px,.9fr) minmax(270px,1.05fr);gap:12px;align-items:stretch}
      .occupancy-v2-unit-options,.occupancy-v2-month,.occupancy-v2-metric{min-height:68px;box-sizing:border-box}
      ...
      .occupancy-v2-unit{flex:1;min-width:0;min-height:58px;border:0;border-radius:13px;background:transparent;color:var(--muted);padding:8px 12px;font-size:.66rem;font-weight:700;cursor:pointer;transition:.16s ease}
      ...
      .occupancy-v2-month{display:flex;align-items:center;justify-content:center;gap:8px;background:var(--surface);border:1px solid var(--border);border-radius:18px;padding:4px 8px;box-shadow:var(--shadow)}
      .occupancy-v2-month strong{flex:1;min-width:0;text-align:center;font-size:.72rem;color:var(--text);white-space:nowrap}
      .occupancy-v2-month .icon-btn{width:44px;height:44px;flex:0 0 44px;border-radius:13px}
      ...
      .occupancy-v2-metric{background:var(--surface);border:1px solid var(--border);border-radius:18px;padding:8px 14px;display:grid;grid-template-columns:38px minmax(0,1fr);align-items:center;gap:11px;box-shadow:var(--shadow)}
      ...
      @media(max-width:1180px){
        .occupancy-v2-toolbar{grid-template-columns:repeat(3,minmax(0,1fr))}
        .occupancy-v2-unit-options{grid-column:1/2}
        .occupancy-v2-month{grid-column:2/4}
      }
      @media(max-width:760px){
        .occupancy-v2-toolbar{grid-template-columns:1fr}
        .occupancy-v2-unit-options,.occupancy-v2-month{grid-column:auto;width:100%}
  ...
  function metrics(){
    ...
    return {today:peopleCount(peopleForDate(_oleiroToday,todayRows)),arrivals:peopleCount(rows.filter(row=>String(row.from||'').slice(0,7)===currentMonth)),departures:peopleCount(rows.filter(row=>String(row.to||'').slice(0,7)===currentMonth))};
  }
  ...
  function html(){
    ...
    return `<section class="occupancy-page-screen occupancy-v2 compact-page-top">
      <header class="occupancy-v2-head"><div class="occupancy-v2-copy"><span class="eyebrow">Ocupação</span><h1>Ocupação da Casa</h1><p>Veja rapidamente quantas pessoas estarão hospedadas, quem chega e quem sai.</p></div></header>
      ...
      <div class="occupancy-v2-toolbar">${metric('fa-house-user',data.today,'Hospedados hoje')}${metric('fa-arrow-right-to-bracket',data.arrivals,'Chegadas no mês')}${metric('fa-arrow-right-from-bracket',data.departures,'Saídas no mês')}${unitSelector()}${monthSelector()}</div>
  ...
  function closeDayPopup(){document.getElementById('occupancyDayModal')?.remove();document.body.classList.remove('occupancy-day-modal-open')}
  function openDayPopup(iso){
    closeDayPopup();
    const rows=peopleForDate(iso),count=peopleCount(rows),root=document.createElement('div');
    root.id='occupancyDayModal';root.className='occupancy-day-modal-backdrop';root.setAttribute('role','presentation');root.onclick=event=>{if(event.target===root)closeDayPopup()};
    root.innerHTML=`<section class="occupancy-day-modal" role="dialog" aria-modal="true" aria-labelledby="occupancyDayModalTitle"><header class="occupancy-day-modal-head"><div><span class="eyebrow">Detalhes do dia</span><h2 id="occupancyDayModalTitle">${esc(longDate(iso))}</h2><p>${count} ${count===1?'pessoa hospedada':'pessoas hospedadas'}</p></div><button class="occupancy-day-modal-close" type="button" onclick="closeOccupancyDayPopup()" aria-label="Fechar"><i class="fa-solid fa-xmark"></i></button></header><div class="occupancy-day-modal-body">${rows.length?rows.map(row=>guest(row,iso)).join(''):'<div class="occupancy-day-modal-empty">Nenhum voluntário hospedado neste dia.</div>'}</div></section>`;
    document.body.appendChild(root);document.body.classList.add('occupancy-day-modal-open');requestAnimationFrame(()=>root.querySelector('.occupancy-day-modal-close')?.focus());
  }
  ...
  async function loadData(){
    ...
    const [monthRows,todayRows]=await Promise.all([monthPromise,todayPromise]);state.occupancyCandidates=monthRows||[];state.occupancyTodayCandidates=todayRows||[];
  }
  async function refresh(){
    closeDayPopup();state.occupancyScreenError='';state.occupancyScreenLoading=true;window.render();
  ...
  window.selectOccupancyDay=function(iso){iso=String(iso||'').slice(0,10);if(!iso)return;state.occupancySelectedDate=iso;window.render();openDayPopup(iso)};
  ...
  window.renderManager=function(){const result=baseRenderManager();if(state.role==='manager'&&state.managerPage==='occupancy')renderPage();else closeDayPopup();return result};
