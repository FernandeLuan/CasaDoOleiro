...
(function occupancyPage(){
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      .occupancy-v2-head{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:28px;min-height:112px}
      .occupancy-v2-copy{min-width:0}
      .occupancy-v2-title{display:flex;align-items:baseline;gap:8px;flex-wrap:wrap;margin:0 0 6px}
      .occupancy-v2-parent{font-size:.66rem;font-weight:700;color:var(--muted)}
      .occupancy-v2-title>i{font-size:.48rem;color:var(--muted);opacity:.75}
      .occupancy-v2-title h1{margin:0;color:var(--text);font-size:clamp(1.55rem,2vw,2.15rem);line-height:1.15;letter-spacing:-.025em}
      .occupancy-v2-copy>p{margin:0;color:var(--muted);font-size:.76rem}
      .occupancy-v2-controls{min-width:310px;display:grid;justify-items:center;align-content:center;gap:9px}
      .occupancy-v2-month{display:flex;align-items:center;justify-content:center;gap:14px;background:var(--surface);border:1px solid var(--border);border-radius:18px;padding:8px 10px;box-shadow:var(--shadow)}
      .occupancy-v2-month strong{min-width:170px;text-align:center;font-size:.78rem;color:var(--text)}
      .occupancy-v2-month .icon-btn{width:38px;height:38px;border-radius:12px}
      .occupancy-v2-legend{display:flex;align-items:center;justify-content:center;gap:12px;flex-wrap:wrap;color:var(--muted);font-size:.58rem}
      ...
      .occupancy-v2-units{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-top:12px}
      .occupancy-v2-unit-label{font-size:.61rem;font-weight:600;color:var(--muted)}
      .occupancy-v2-unit-options{display:flex;gap:5px;flex-wrap:wrap}
      .occupancy-v2-unit{min-height:32px;border:1px solid var(--border);border-radius:11px;background:var(--surface);color:var(--muted);padding:6px 11px;font-size:.61rem;font-weight:700;cursor:pointer;transition:.16s ease}
      .occupancy-v2-unit:hover{border-color:var(--border-strong);color:var(--text)}
      .occupancy-v2-unit.active{background:var(--primary-soft);border-color:color-mix(in srgb,var(--primary) 36%,var(--border));color:var(--primary)}
      ...
      @media(max-width:1023px){
        .occupancy-v2-head{grid-template-columns:1fr;min-height:0;gap:16px;align-items:start}.occupancy-v2-controls{min-width:0;width:100%;justify-items:stretch}.occupancy-v2-month{width:100%}.occupancy-v2-month strong{flex:1}.occupancy-v2-legend{justify-content:center}
  ...
  function unitSelector(){const current=unitId(),rows=units();return `<div class="occupancy-v2-units"><span class="occupancy-v2-unit-label">Unidade</span><div class="occupancy-v2-unit-options">${rows.map(row=>`<button class="occupancy-v2-unit ${String(row.id)===current?'active':''}" type="button" onclick="selectOccupancyUnit('${escapeHtml(String(row.id))}')">${escapeHtml(row.name||row.id)}</button>`).join('')}</div></div>`}
