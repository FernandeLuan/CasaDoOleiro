...
async function openPerson(id,tab='overview'){
  ...
  openModal(p.name,`${p.country} • ${p.unit}`,`<div class="tabs person-tabs">${tabs.map(([k,l])=>`<button class="tab ${tab===k?'active':''}" onclick="openPerson(${JSON.stringify(p.id)},'${k}')">${l}</button>`).join('')}</div>${personTabContent(p,tab)}`)
}
