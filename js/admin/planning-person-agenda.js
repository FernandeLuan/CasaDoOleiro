...
(function planningPersonAgenda(){
  ...
  function feedbackAuthor(){const session=state.currentSession||{},user=session.user||{},email=String(session.email||user.email||'').trim(),name=knownFeedbackAuthorName(user.displayName||user.name)||knownFeedbackAuthorName(email);return {uid:String(session.uid||''),name:(name||'Autor não registrado').slice(0,120)}}
  function feedbackHtml(s){const value=String(s?.feedback||'').trim();if(!value)return '';const author=knownFeedbackAuthorName(s?.feedbackAuthorName)||'Autor não registrado',when=feedbackWhen(s?.feedbackUpdatedAt),visibility=s?.feedbackInternal===true?'<em class="planning-feedback-internal-badge">Interno</em>':'';return `<section class='planning-activity-feedback' aria-label='Feedback da atividade'><div class='planning-feedback-heading'><strong>Feedback:</strong>${visibility}</div><p data-no-i18n>${escapeHtml(value)}</p><span>${escapeHtml(author)}${when?` · ${escapeHtml(when)}`:''}</span></section>`}
  ...
  window.planningOpenFeedback=async function(encodedApplicationId,encodedSessionId){
    ...
    openModal(title,escapeHtml(sessionName(s)),`<div class='field planning-feedback-modal-field'><label for='planningActivityFeedback'>Feedback da atividade</label><textarea id='planningActivityFeedback' class='textarea' rows='5' maxlength='2000' placeholder='Escreva um feedback sobre esta realização da atividade.'>${escapeHtml(current)}</textarea></div><label class='planning-feedback-internal-option'><span class='planning-feedback-internal-copy'><strong>Somente interno</strong><small>Ativado: somente a equipe vê. Desativado: o voluntário também vê.</small></span><input id='planningFeedbackInternal' type='checkbox' ${s.feedbackInternal===true?'checked':''}><span class='planning-feedback-switch' aria-hidden='true'></span></label>`,`<div class='planning-feedback-modal-actions'><button class='btn btn-outline' type='button' onclick='closeModal()'>Cancelar</button><button id='planningFeedbackSave' class='btn btn-primary' type='button' onclick="planningSaveFeedback('${safe(applicationId)}','${safe(sessionId)}')">Salvar feedback</button></div>`);
  ...
  window.planningSaveFeedback=async function(encodedApplicationId,encodedSessionId){
    ...
    try{
      await window.OleiroServices.planning.updateSession(sessionId,{feedback:text,feedbackAuthorUid:actor.uid,feedbackAuthorName:actor.name,feedbackUpdatedAt:new Date(),feedbackInternal:internal});
      try{await window.OleiroServices.history?.append?.(applicationId,existed?'activity_feedback_updated':'activity_feedback_added',{unitId:s.unitId||'',metadata:{sessionId:String(sessionId),activityName:sessionName(s),visibility:internal?'internal':'shared'}})}catch(error){console.warn('Feedback salvo, mas o histórico não pôde ser registrado:',error)}
