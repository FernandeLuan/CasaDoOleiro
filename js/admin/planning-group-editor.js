...
(function planningGroupEditor(){
  ...
  window.planningOpenGroup=async function(encodedApplicationId,encodedSessionId){
    ...
    const body=`<div class="field"><label>Grupos</label><div class="check-grid">${options.map(group=>`<label class="check-card"><input type="checkbox" data-planning-group value="${escapeHtml(group)}" ${selected.includes(group)?'checked':''}><span>${escapeHtml(group)}</span></label>`).join('')}</div></div>`;
