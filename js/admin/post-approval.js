...
(function postApprovalAdmin(){
  ...
  window.reviewPostApprovalProposal=async function(encodedApplicationId,encodedActivityId,decision){
    ...
    try{const result=await window.OleiroServices.planning.reviewPostApprovalProposal({applicationId,activityId,decision,note});invalidateCandidatePlanning?.(applicationId);invalidateManagerPendingChanges?.();invalidateManagerScheduleCache?.();if(result?.countDelta){p.sessions=Math.max(0,(Number(p.sessions)||0)+(Number(result.countDelta.sessionCount)||0));p.activities=Math.max(0,(Number(p.activities)||0)+(Number(result.countDelta.activityCount)||0))}await hydrateManagerPendingChanges({force:true});await hydrateCandidatePlanning(applicationId,{force:true});renderPersonModal(p,'plan');if(decision==='approve')showToast('Nova atividade aprovada.');else if(decision==='reject')showToast('Nova atividade recusada.');else showToast('Reajuste enviado ao voluntário.');hydrateManagerDashboardData?.().catch(console.error)}catch(error){console.error(error);showToast(error?.message||'Não foi possível revisar a atividade.')}
  };
