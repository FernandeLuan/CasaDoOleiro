...
(function adminEmergencyContact(){
  ...
  function emergencyCard(p){
    ...
    return `<div class="card account-emergency-card"><div class="account-card-head"><div><span class="eyebrow">${escapeHtml(text('emergency.title'))}</span></div></div>${rows}</div>`;
  }
  ...
  window.openVolunteerEmergencyEditor=function(encodedId,index){
    const id=decodeURIComponent(encodedId),p=candidateById(id);if(!p)return;const i=Number(index),row=participantContact(p,i),has=hasContact(row);
