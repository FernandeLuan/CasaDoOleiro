...
(function refinementsR20Admin(){
  ...
  function sessionFormBody(session=null,date=''){
    ...
    return `<div class="form-grid manager-activity-form"><div class="field"><label for="managerActName">Nome da atividade</label><input id="managerActName" class="input" value="${escapeHtml(session?.activityName||a.name||'')}" placeholder="Ex.: Conversação em espanhol"></div><div class="field"><label for="managerActDesc">Descrição</label><textarea id="managerActDesc" class="textarea" placeholder="Como a atividade será realizada?">${escapeHtml(session?.activityDescription||a.description||'')}</textarea></div><div class="field-row"><div class="field"><label for="managerActDuration">Duração</label><input id="managerActDuration" class="input" type="number" min="15" max="240" step="15" value="${Number(session?.duration||a.duration)||60}"></div><div class="field"><label for="managerActGroup">Grupo</label><select id="managerActGroup" class="select">${groupOptions(group)}</select></div></div><div class="field-row"><div class="field"><label for="managerActPeriod">Período</label><select id="managerActPeriod" class="select">${['Manhã','Tarde','Noite','Sem preferência'].map(value=>`<option ${String(session?.period||a.period||'Sem preferência')===value?'selected':''}>${value}</option>`).join('')}</select></div><div class="field"><label for="managerActTime">Horário</label><input id="managerActTime" class="input" type="time" value="${escapeHtml(session?.time||a.time||'15:15')}"></div></div><div class="field"><label for="managerActMaterials">Materiais necessários</label><input id="managerActMaterials" class="input" value="${escapeHtml(session?.materials||a.materials||'')}" placeholder="Nenhum, cartolina, bola..."></div><div class="field"><label for="managerActNotes">Observações</label><textarea id="managerActNotes" class="textarea" placeholder="Opcional">${escapeHtml(session?.notes||a.notes||'')}</textarea></div>${date?`<div class="manager-activity-date"><i class="fa-regular fa-calendar"></i><span>${escapeHtml(dayName(date))} • ${escapeHtml(fmtDate(date,true))}</span></div>`:''}</div>`;
  }
  function readManagerActivityForm(){
    const group=document.getElementById('managerActGroup')?.value||'Livre';
    return {name:document.getElementById('managerActName')?.value.trim()||'',description:document.getElementById('managerActDesc')?.value.trim()||'',duration:Number(document.getElementById('managerActDuration')?.value)||60,groupId:group,participation:participationForGroup(group),materials:document.getElementById('managerActMaterials')?.value.trim()||'Nenhum',notes:document.getElementById('managerActNotes')?.value.trim()||'',period:document.getElementById('managerActPeriod')?.value||'Sem preferência',time:document.getElementById('managerActTime')?.value||''};
  }
  ...
  function updateAdminPlanCacheAfterCreate(p,date,result){
    ...
    Object.entries(state.adminPlanPageCache||{}).forEach(([key,cache])=>{if(!key.startsWith(`${p.id}|`)||!Array.isArray(cache?.dates)||!cache.dates.includes(date))return;cache.activities=cache.activities||[];if(!cache.activities.some(item=>String(item.id)===String(activity.id)))cache.activities.push(activity);cache.sessions=(cache.sessions||[]).concat(rows).sort((a,b)=>String(a.date||'').localeCompare(String(b.date||''))||String(a.time||'').localeCompare(String(b.time||'')));cache.at=Date.now()});
  ...
  window.saveAdminEditPlanningSession=async function(encodedId,encodedSessionId,encodedDate){
    ...
    try{const patch=await window.OleiroServices.planning.managerUpdateSession({sessionId,activityId:session.activityId,patch:{activityName:data.name,activityDescription:data.description,duration:data.duration,participation:data.participation,materials:data.materials,notes:data.notes,period:data.period,time:data.time,groupId:data.groupId}});updateSessionEverywhere(id,sessionId,patch);renderPersonModal(p,'plan');const target=modalRoot.querySelector(`details[data-plan-date="${CSS.escape(date)}"]`);if(target)target.open=true;showToast('Atividade atualizada.')}catch(error){console.error(error);showToast(error?.message||'Não foi possível editar a atividade.');if(button?.isConnected){button.disabled=false;button.textContent='Salvar alterações'}}
  };
  ...
  window.requestAdminPlanningAdjustment=function(encodedId,encodedSessionId,encodedDate){
    ...
    openModal('Solicitar ajuste',`${escapeHtml(name)} • ${escapeHtml(fmtDate(date,true))}`,`<div class="field"><label for="adminSessionAdjustNote">Orientação ao candidato</label><textarea id="adminSessionAdjustNote" class="textarea" placeholder="Ex.: alterar o horário, reduzir a duração..."></textarea></div>`,`<button id="adminSessionAdjustSave" class="btn btn-primary btn-block" type="button" onclick="saveAdminPlanningAdjustment('${safe(id)}','${safe(date)}')">Enviar ajuste</button>`);
  };
