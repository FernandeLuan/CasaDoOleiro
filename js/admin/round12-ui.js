...
(function round12Admin(){
  ...
  adminPlanningDayCard=function(p,day){
    ...
    const rows=day.sessions.map(session=>{
      ...
      return `<div class="planning-session-row"><div><div class="planning-session-title"><strong>${escapeHtml(a.name||session.activityName||'Atividade')}</strong>${info}</div><span>${Number(session.duration||a.duration)||0} min • ${escapeHtml(activityPeriodValue(session,a))}${group}</span>${description?`<p class="admin-session-description"><strong>Descrição:</strong> ${escapeHtml(description)}</p>`:''}${change?`<button class="btn btn-soft approve-change-button" type="button" onclick="approveVolunteerChange('${safe(session.id)}','${safe(p.id)}')"><i class="fa-solid fa-check"></i>Aprovar mudança</button>`:''}</div></div>`;
    }).join('');
