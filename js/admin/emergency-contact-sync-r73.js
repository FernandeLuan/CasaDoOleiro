...
(function emergencyContactSync(){
  ...
  async function reloadEmergencyProfiles(p){
    ...
    p.emergencyProfilesLoaded=true;p.emergencyProfilesLoading=false;
  }
  ...
  async function rebuildAccount(id){
    ...
    state.managerPlanningTab='account';state.managerPlanningBody='';state.managerPlanningLoading=true;if(typeof closeModal==='function')closeModal();
    try{if(typeof window.openPerson==='function')await window.openPerson(String(p.id),'account')}
    catch(error){console.error('Falha ao reconstruir Conta após contato de emergência:',error)}
    finally{state.managerPlanningLoading=false;if(state.managerPage==='planning'&&String(state.managerPlanningPersonId)===String(p.id)&&typeof render==='function')render()}
  }

  const baseSave=window.saveVolunteerEmergencyContact;
  if(typeof baseSave==='function'){
    window.saveVolunteerEmergencyContact=async function(encodedId,index){const id=decodeURIComponent(encodedId);await baseSave(encodedId,index);await rebuildAccount(id)};
  ...
  if(typeof baseClear==='function'){
    window.clearVolunteerEmergencyContact=async function(encodedId,index){const id=decodeURIComponent(encodedId);await baseClear(encodedId,index);await rebuildAccount(id)};
