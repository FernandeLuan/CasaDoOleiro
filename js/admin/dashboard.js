...
function managerHome(){
  ...
  return `<style id="managerHomeStyles">
    ...
    @media(min-width:1024px){
      ...
      .manager-home-card{padding:22px 24px;min-height:0}.manager-home-pending{display:flex;flex-direction:column;justify-content:flex-start}.manager-home-pending .pending-grid{grid-template-columns:1fr 1fr;margin-top:auto;margin-bottom:auto}.manager-home-pending .pending-grid .metric{min-height:92px;display:grid;grid-template-columns:46px minmax(0,1fr);gap:12px;align-items:center;text-align:left!important;padding:14px 16px;border-radius:18px}.manager-home-pending .pending-grid .metric-icon{margin:0;width:46px;height:46px;border-radius:14px}.manager-home-pending .pending-grid .metric>div:last-child{width:auto}.manager-home-pending .pending-grid .metric strong{text-align:left;font-size:1.08rem}.manager-home-pending .pending-grid .metric span{text-align:left;margin-top:2px;font-size:.68rem}
  ...
  <div class="manager-home">
    ...
    <div class="manager-home-grid manager-home-bottom">
      <section class="manager-home-card manager-home-today"><div class="section-head"><div><h2>Hoje na Casa</h2><p>${longDate(_oleiroToday)}</p></div></div><div class="list manager-home-today-list">${todaySessions.length?todaySessions.map(s=>agendaItem(s.activity.name,s.activity.owner,s.group,s.status,activityPeriodValue(s.raw||{},s.activity),s.activity.duration)).join(''):'<div class="empty">Nenhuma atividade prevista para hoje.</div>'}</div></section>
