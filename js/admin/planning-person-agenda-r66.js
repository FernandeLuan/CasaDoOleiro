...
(function planningPersonAgenda(){
  ...
  function dayCardHtml(p,date,sessions){
    ...
    const total=sessions.reduce((sum,row)=>sum+rowMinutes(row),0),parts=weekdayParts(date),count=sessions.length;const summary=count?`${count} ${count===1?'atividade':'atividades'} · ${minutesLabel(total)}`:'Nenhuma atividade planejada';let inner=prepareDayContent(content,p,sessions);
