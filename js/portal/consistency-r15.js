...
(function consistencyR15Portal(){
  ...
  window.moveSessionById=function(encodedId,byVolunteer=false){
    ...
    openModal(approved?'Solicitar mudança':'Mover sessão',`${escapeHtml(a.name||session.activityName||'Atividade')} • atual: ${fmtDate(currentDate,true)}`,`<div class="field"><label>Nova data</label><select id="moveDate" class="select move-date-select">${dates.map(date=>`<option value="${date}" ${date===currentDate?'selected':''}>${dayName(date)} • ${fmtDate(date,true)}${date===currentDate?' — atual':''}</option>`).join('')}</select></div><div class="field" style="margin-top:10px"><label>Novo horário sugerido</label><input id="moveTime" class="input" type="time" value="${escapeHtml(session.time||a.time||'')}"></div>`,`<button id="moveSessionSave" class="btn btn-primary btn-block" type="button" onclick="saveMoveBySessionId('${safe(id)}',${byVolunteer})">${approved?'Enviar para análise':'Mover'}</button>`);
  };
  ...
  window.saveMoveBySessionId=async function(encodedId,byVolunteer=false){
    ...
    const button=document.getElementById('moveSessionSave');if(button){button.disabled=true;button.innerHTML=`<i class="fa-solid fa-circle-notch fa-spin"></i> ${approved?'Enviando...':'Movendo...'}`}
    try{await window.OleiroServices.planning.updateSession(session.id,patch);Object.assign(session,patch);closeModal();render();showToast(approved?'Mudança enviada para análise.':'Cronograma atualizado.')}catch(error){console.error(error);showToast(error?.message||'Não foi possível atualizar a sessão.');if(button?.isConnected){button.disabled=false;button.textContent=approved?'Enviar para análise':'Mover'}}
  };
