...
(function consistencyR15Portal(){
  ...
  sessionCardVolunteer=function(s,editable){
    ...
    return `<div class="activity-card volunteer-session-card"><div class="activity-row"><div class="volunteer-session-main"><div class="volunteer-session-title"><h4>${escapeHtml(a.time||raw.time||'—')} • ${escapeHtml(a.name||raw.activityName||'Atividade')}</h4>${info}</div><p>${Number(a.duration||raw.duration)||0} min • ${escapeHtml(typeof tValue==='function'?tValue(a.period||raw.period||'Sem preferência'):a.period||raw.period||'Sem preferência')}</p>${description?`<p class="volunteer-session-description"><strong>${escapeHtml(tx('portal.activity.descriptionLabel','Descrição:'))}</strong> ${escapeHtml(description)}</p>`:''}</div></div>${actions}</div>`;
  };
  ...
  window.saveMoveBySessionId=async function(encodedId,byVolunteer=false){
    ...
    if(!newDate)return showToast(tx('portal.move.chooseDate','Escolha a nova data.'));if(newDate===oldDate&&newTime===oldTime)return showToast(tx('portal.move.changeRequired','Altere a data ou o horário antes de enviar.'));
