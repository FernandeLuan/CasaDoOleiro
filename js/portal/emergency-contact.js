...
(function portalEmergencyContact(){
  ...
  function cardHtml(){
    ...
    const actionLabel=has?text('emergency.edit'):text('emergency.add');
    const action=`<button class="btn btn-outline btn-xs emergency-edit-icon" type="button" onclick="openMyEmergencyContactEditor()" aria-label="${escapeHtml(actionLabel)}" title="${escapeHtml(actionLabel)}"><i class="fa-solid fa-pen" aria-hidden="true"></i></button>`;
  ...
  window.syncMyEmergencySaveState=function(){
    ...
    const complete=['myEmergencyName','myEmergencyRelationship','myEmergencyPhone'].every(id=>String(document.getElementById(id)?.value||'').trim());
    button.disabled=!complete;
  };

  window.openMyEmergencyContactEditor=function(){
    const row=normalize(state.currentSession?.profile?.emergencyContact),has=hasContact(row);
    const body=`<div class="form-grid"><div class="field"><label for="myEmergencyName">${escapeHtml(text('emergency.name'))}</label><input id="myEmergencyName" class="input" required aria-required="true" value="${escapeHtml(row.name)}" placeholder="${escapeHtml(text('emergency.namePlaceholder'))}" oninput="syncMyEmergencySaveState()"></div><div class="field"><label for="myEmergencyRelationship">${escapeHtml(text('emergency.relationship'))}</label><input id="myEmergencyRelationship" class="input" required aria-required="true" value="${escapeHtml(row.relationship)}" placeholder="${escapeHtml(text('emergency.relationshipPlaceholder'))}" oninput="syncMyEmergencySaveState()"></div><div class="field"><label for="myEmergencyPhone">${escapeHtml(text('emergency.phone'))}</label><input id="myEmergencyPhone" class="input" type="tel" required aria-required="true" value="${escapeHtml(row.phone)}" placeholder="${escapeHtml(text('emergency.phonePlaceholder'))}" oninput="syncMyEmergencySaveState()"></div></div>`;
    ...
    openModal(text('emergency.title'),'',body,footer);syncMyEmergencySaveState();
  };
  ...
  window.saveMyEmergencyContact=async function(){
    ...
    const row={name:document.getElementById('myEmergencyName')?.value.trim()||'',relationship:document.getElementById('myEmergencyRelationship')?.value.trim()||'',phone:document.getElementById('myEmergencyPhone')?.value.trim()||''};if(!row.name||!row.relationship||!row.phone){syncMyEmergencySaveState();return showToast(text('emergency.required'))}
