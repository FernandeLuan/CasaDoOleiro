...
(function postApprovalPortal(){
  ...
  sessionCardVolunteer=function(s,editable){
    ...
    return `<div class="activity-card volunteer-session-card post-approval-proposal"><div class="activity-row"><div class="volunteer-session-main"><div class="volunteer-session-title"><h4>${escapeHtml(a.time||raw.time||'—')} • <span data-no-i18n>${escapeHtml(a.name||raw.activityName||'Atividade')}</span></h4>${info}</div><p>${Number(a.duration||raw.duration)||0} min • ${escapeHtml(tValue(a.period||raw.period||'Sem preferência'))}</p>${description?`<p class="volunteer-session-description"><strong>${escapeHtml(t('portal.activity.descriptionLabel'))}</strong> <span data-no-i18n>${escapeHtml(description)}</span></p>`:''}</div>${badge(meta[0],meta[1])}</div>${note}${actions}</div>`;
  };
