...
(function postApprovalPortal(){
  ...
  volunteerAgenda=function(){return `<section class="section"><div class="section-head planning-share-section-head"><div><span class="eyebrow">${escapeHtml(t('portal.agenda.eyebrow'))}</span><h2>${escapeHtml(t('portal.agenda.title'))}</h2><p>${escapeHtml(t('portal.agenda.subtitle'))}</p></div>${typeof portalPlanningShareButton==='function'?portalPlanningShareButton():''}</div><div class="notice"><i class="fa-solid fa-lightbulb"></i><div>${escapeHtml(t('portal.agenda.newIdea'))}</div></div><div style="margin-top:14px">${volunteerAgendaContent(true)}</div></section>`};
