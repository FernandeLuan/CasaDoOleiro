...
(function uxR16Portal(){
  ...
  window.moveSessionById=function(encodedId,byVolunteer=false){
    ...
    openModal(title,`${escapeHtml(a.name||session.activityName||'Atividade')} • atual: ${fmtDate(currentDate,true)}`,`<div class="change-request-card"><div class="field"><label>Nova data</label><select id="moveDate" class="select move-date-select">${dates.map(date=>`<option value="${date}" ${date===currentDate?'selected':''}>${dayName(date)} • ${fmtDate(date,true)}${date===currentDate?' — atual':''}</option>`).join('')}</select></div><div class="field"><label>Novo horário sugerido</label><input id="moveTime" class="input" type="time" value="${escapeHtml(session.time||a.time||'')}"></div>${approved?`<div class="field"><label>Descrição da mudança</label><textarea id="moveReason" class="textarea" placeholder="Ex.: preciso antecipar esta atividade porque terei outra atividade no horário atual."></textarea><small>Ajuda a equipe a entender o motivo da solicitação.</small></div>`:''}</div>`,`<button id="moveSessionSave" class="btn btn-primary btn-block" type="button" onclick="saveMoveBySessionId('${safe(id)}',${byVolunteer})">${approved?'Enviar para análise':'Salvar mudança'}</button>`);
  };
