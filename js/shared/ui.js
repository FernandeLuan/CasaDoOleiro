...
async function logout(){
  ...
  try{if(!dev&&window.OleiroAuth?.signOut)await withGlobalLoading(()=>window.OleiroAuth.signOut())}catch(error){console.error('Falha ao encerrar sessão:',error)}finally{
    sessionStorage.removeItem('oleiro-role');sessionStorage.removeItem('oleiro-volunteer-mode');sessionStorage.removeItem('oleiro-volunteer-plan-status');location.replace(document.body.dataset.root||'../index.html');
  }
}
