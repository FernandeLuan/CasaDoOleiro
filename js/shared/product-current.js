...
(function productCurrentShared(){
  ...
  function sessionDefinition(session){
    return session?.activity||(state.activities||[]).find(row=>String(row.id)===String(session?.activityId))||{
      id:session?.activityId,name:session?.activityName||'Atividade',time:session?.time||''
    };
  }
  window.moveSessionById=function(sessionId,byVolunteer=false){
    ...
    const activity=sessionDefinition(session),currentDate=String(session.date||'');
    const options=moveDatesForSession(session).filter(date=>date!==currentDate);
  ...
  window.saveMoveBySessionId=async function(encodedId,byVolunteer=false){
    ...
    try{
      ...
      if(state.role==='manager'){
        invalidateManagerScheduleCache?.();const cache=typeof candidatePlanningCache==='function'?candidatePlanningCache(session.applicationId):null;
        if(cache){const cached=cache.sessions.find(row=>String(row.id)===String(session.id));if(cached)Object.assign(cached,patch)}
      }
