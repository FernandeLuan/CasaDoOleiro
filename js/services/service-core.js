(function initOleiroServiceCore(){
  ...
  function recordQuery(name,started,count,meta={}){
    const details={...(meta||{})};
    const row={name:String(name||'firestore/query'),ms:Math.max(0,Date.now()-Number(started||Date.now())),count:Number(count)||0,...details,meta:details,at:new Date().toISOString()};
    ...
    window.OleiroMonitoring?.recordQueryMetric?.(row);
    if(row.ms>1200)console.warn(`[Firestore lento] ${row.name}: ${row.ms}ms • ${row.count} docs`,details);
    return row;
  }
