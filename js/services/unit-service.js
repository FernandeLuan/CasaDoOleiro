(function initUnitService(){
  ...
  services.units={
    ...
    async update(id,patch={}){return services.run(async()=>{const context=await services.firebase();const {firestore}=context.modules;const clean={...patch,updatedAt:firestore.serverTimestamp()};await firestore.updateDoc(firestore.doc(context.db,'units',String(id)),clean);return {id:String(id),...patch};},{loading:false});}
  };
})();
