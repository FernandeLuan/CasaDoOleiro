...
test('Pre-approval keeps normal actions separate from an activity-specific adjustment',async({page})=>{
  ...
  const day=modal.locator('details[data-plan-date="2026-09-15"]');await expect(day).toBeVisible({timeout:20_000});await day.locator('summary').click();let card=day.locator('.admin-portal-activity-card').filter({hasText:'Oficina candidato E2E'});
  ...
  await card.getByRole('button',{name:/Pedir ajuste/}).click();await page.locator('#r31SessionAdjustNote').fill('Alterar somente o horário desta atividade.');await page.locator('#r31SessionAdjustSave').click();
  const reopened=modal.locator('details[data-plan-date="2026-09-15"]');card=reopened.locator('.admin-portal-activity-card').filter({hasText:'Oficina candidato E2E'});await expect(card).toHaveClass(/r31-card-warning/);await expect(card.locator('.r31-review-details')).toContainText('Alterar somente o horário desta atividade.');await expect(card.locator('.r32-session-signal')).toHaveCount(0);
  ...
  await relogin(page,'voluntario@oleiro.test','Volunteer123!','portal');await navAction(page,'Planejamento').click();let volunteerCard=page.locator('.activity-card').filter({hasText:'Oficina candidato E2E'});await expect(volunteerCard.locator('.r31-review-details')).toContainText('Alterar somente o horário desta atividade.');await expect(volunteerCard.locator('.r32-session-signal')).toHaveCount(0);await volunteerCard.getByRole('button',{name:/Ajustar atividade/}).click();await page.locator('#r31ActTime').fill('16:00');await page.locator('#r31VolunteerAdjustSave').click();
  ...
  await page.getByRole('button',{name:/Reenviar planejamento/}).click();await expect.poll(()=>page.evaluate(()=>{const s=state.sessions.find(row=>row.id==='e2e-candidate-session');return `${state.currentApplication?.status}|${s?.adminAdjustmentStatus}|${s?.time}`}),{timeout:20_000}).toBe('analysis|analysis|16:00');
});
