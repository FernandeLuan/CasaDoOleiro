// Round 32/33 focused regression: activity-scoped reasons, separated pre/post approval actions and ready state.
...
test('Pre-approval keeps normal actions separate from an activity-specific adjustment',async({page})=>{
  ...
  const day=planning.locator('details[data-plan-date="2026-09-15"]');await expect(day).toBeVisible({timeout:20_000});await day.locator('summary').click();let card=day.locator('.admin-portal-activity-card').filter({hasText:'Oficina candidato E2E'});
  const management=card.locator('.admin-session-manage-actions');await expect(management).toBeVisible();await expect(management.getByRole('button',{name:/Editar/})).toBeVisible();await expect(management.getByRole('button',{name:/Mover/})).toBeVisible();await expect(management.getByRole('button',{name:/Excluir/})).toBeVisible();await expect(management.getByRole('button')).toHaveCount(3);
  ...
  const reopened=planning.locator('details[data-plan-date="2026-09-15"]');card=reopened.locator('.admin-portal-activity-card').filter({hasText:'Oficina candidato E2E'});await expect(card).toHaveClass(/r31-card-warning/);await expect(card.locator('.r31-review-details')).toContainText('Alterar somente o período desta atividade.');await expect(card.locator('.r32-session-signal')).toHaveCount(0);
  ...
  await relogin(page,'voluntario@oleiro.test','Volunteer123!','portal');await navAction(page,'Planejamento').click();let volunteerCard=page.locator('.activity-card').filter({hasText:'Oficina candidato E2E'});await expect(volunteerCard.locator('.r31-review-details')).toContainText('Alterar somente o período desta atividade.');await expect(volunteerCard.locator('.r32-session-signal')).toHaveCount(0);await volunteerCard.getByRole('button',{name:/Ajustar atividade/}).click();await page.locator('#r31ActPeriod').selectOption('Noite');await page.locator('#r31VolunteerAdjustSave').click();
