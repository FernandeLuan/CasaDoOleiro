...
test('monitoring catches asset failures and avoids cold-start Firestore noise', async () => {
  ...
  expect(monitoring).toContain('slowQueryReportMs=2500');
  expect(monitoring).toContain('slowQueryCriticalMs=4000');
  expect(monitoring).toContain('coldStart&&durationMs<slowQueryCriticalMs');
