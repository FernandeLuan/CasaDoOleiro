...
test('post-approval activity returned for readjustment stays horizontal and resubmits without permission error',async({page})=>{
  ...
  await expect.poll(async()=>{const row=await sessionState(page,'e2e-approved-application',activityName);return row?`${row.reviewStatus}|${row.time}|${row.period}`:''},{timeout:20_000}).toBe('analysis|16:45|Tarde');
});
...
test('approving planning resolves Ajustado instead of carrying the tag into approved planning',async({page})=>{
  ...
  await relogin(page,'voluntario@oleiro.test','Volunteer123!','portal');await navAction(page,'Planejamento').click();card=page.locator('.activity-card').filter({hasText:'Oficina candidato E2E'});await expect(card.getByRole('button',{name:/Ajustar atividade/})).toBeVisible({timeout:20_000});await card.getByRole('button',{name:/Ajustar atividade/}).click();await page.locator('#r31ActTime').fill('16:00');await page.locator('#r31VolunteerAdjustSave').click();await expect(page.locator('.activity-card').filter({hasText:'Oficina candidato E2E'})).toContainText('Ajustado');
