...
async function ensureDay(detail,date){const day=detail.locator(`.planning-person-day[data-plan-date="${date}"]`);await expect(day).toBeVisible({timeout:20_000});return day}
...
test('post-approval activity returned for readjustment stays horizontal and resubmits without permission error',async({page})=>{
  ...
  await relogin(page,'admin@oleiro.test','Admin123!','admin');const modal=await openVolunteer(page,'approved','Aprovado E2E');const day=await ensureDay(modal,'2026-09-23');let card=day.locator('.admin-portal-activity-card').filter({hasText:activityName});await expect(card).toBeVisible();await openAdminActions(card);await card.getByRole('button',{name:/Reajustar$/}).click();await page.locator('#planningPostAdjustment').fill('Trocar o período antes de aprovar.');await page.locator('#modalRoot').getByRole('button',{name:/Enviar reajuste/}).click();
  await expect(page.locator('#planningPostAdjustment')).toHaveCount(0,{timeout:20_000});
...
test('approving planning resolves Ajustado instead of carrying the tag into approved planning',async({page})=>{
  ...
  await relogin(page,'admin@oleiro.test','Admin123!','admin');let modal=await openVolunteer(page,'analysis','Voluntário E2E');let day=await ensureDay(modal,'2026-09-15');let card=day.locator('.admin-portal-activity-card').filter({hasText:'Oficina candidato E2E'});await openAdminActions(card);await card.getByRole('button',{name:/Pedir ajuste/}).click();await page.locator('#r31SessionAdjustNote').fill('Trocar o período.');await page.locator('#r31SessionAdjustSave').click();
  await expect(page.locator('#r31SessionAdjustNote')).toHaveCount(0,{timeout:20_000});
