...
test('post-approval activity returned for readjustment stays horizontal and resubmits without permission error',async({page})=>{
  ...
  await relogin(page,'admin@oleiro.test','Admin123!','admin');const modal=await openVolunteer(page,'approved','Aprovado E2E');const day=await ensureDay(modal,'2026-09-23');let card=day.locator('.admin-portal-activity-card').filter({hasText:activityName});await expect(card).toBeVisible();await openAdminActions(card);await card.getByRole('button',{name:/Reajustar$/}).click();await page.locator('#postApprovalReajustNote').fill('Trocar o período antes de aprovar.');await page.locator('#modalRoot').getByRole('button',{name:/Enviar reajuste/}).click();
