...
test('approving planning resolves Ajustado instead of carrying the tag into approved planning',async({page})=>{
  ...
  await relogin(page,'admin@oleiro.test','Admin123!','admin');planning=await openAdminPlanning(page,'Voluntário E2E');day=await ensureDay(planning,'2026-09-15');await expect(day.locator('.admin-portal-activity-card').filter({hasText:'Oficina candidato E2E'})).toContainText('Ajustado');const approve=planning.getByRole('button',{name:/Aprovar$/}).last();await expect(approve).toBeVisible();await approve.click();await page.locator('#approvePlanningConfirmR25').click();
