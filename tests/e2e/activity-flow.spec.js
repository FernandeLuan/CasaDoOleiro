...
test('Admin creates same activity at two times with independent multi-group selections',async({page})=>{
  ...
  const activityRows=modal.locator('.planning-session-row').filter({hasText:'Oficina repetida E2E'});await expect(activityRows).toHaveCount(2);await expect(activityRows.first()).toContainText('Descrição visível no card');const descriptionCount=await activityRows.first().evaluate((row)=>{const value='Descrição visível no card';return row.innerText.split(value).length-1});expect(descriptionCount).toBe(1);await expect(activityRows.first()).toContainText('Observação visível');await expect(activityRows.first()).toContainText('Cartolina');await expect(activityRows.first().locator('.planning-note-button')).toHaveCount(0);
  const detailOrder=await activityRows.first().evaluate(row=>{const text=row.innerText;return {notes:text.indexOf('Observação visível'),description:text.indexOf('Descrição visível no card'),materials:text.indexOf('Cartolina'),edit:text.indexOf('Editar')}});expect(detailOrder.notes).toBeGreaterThanOrEqual(0);expect(detailOrder.description).toBeGreaterThan(detailOrder.notes);expect(detailOrder.materials).toBeGreaterThan(detailOrder.description);expect(detailOrder.edit).toBeGreaterThan(detailOrder.materials);
});
