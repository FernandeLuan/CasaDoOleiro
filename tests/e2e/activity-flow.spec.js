...
async function openPendingVolunteer(page){
  ...
  await expect(list.getByText(/Carregando voluntários/)).toHaveCount(0,{timeout:20_000});const candidate=list.locator('.list-item.clickable').filter({hasText:'Voluntário E2E'}).first();await expect(candidate).toBeVisible({timeout:20_000});await candidate.click();
...
test('Admin creates same activity in two periods with independent multi-group selections',async({page})=>{
  await login(page,'admin@oleiro.test','Admin123!','admin');const planning=await openPendingVolunteer(page);const day=planning.locator('details[data-plan-date]').first();await day.locator('summary').click();await expect(day).toHaveAttribute('open','');const add=day.getByRole('button',{name:/Adicionar atividade$/}).first();await expect(add).toBeVisible({timeout:20_000});await add.click();
