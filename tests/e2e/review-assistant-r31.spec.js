...
test('Admin adjustment stays on one activity and volunteer resubmits that session',async({page})=>{
  ...
  const reopened=modal.locator('details[data-plan-date="2026-09-15"]');await expect(reopened).toHaveAttribute('open','');card=reopened.locator('.admin-portal-activity-card').filter({hasText:'Oficina candidato E2E'});await expect(card).toHaveClass(/r31-card-warning/);await expect(card).toContainText('Motivo do ajuste:');await expect(card).toContainText('Alterar o horário desta atividade.');await expect(card.locator('.r32-session-signal')).toHaveCount(0);
  ...
  await relogin(page,'voluntario@oleiro.test','Volunteer123!','portal');await navAction(page,'Planejamento').click();let vcard=page.locator('.activity-card').filter({hasText:'Oficina candidato E2E'});await expect(vcard).toHaveClass(/r31-card-warning/);await expect(vcard).toContainText('Motivo do ajuste:');await expect(vcard).toContainText('Alterar o horário desta atividade.');await expect(vcard.locator('.r32-session-signal')).toHaveCount(0);
...
test('Legacy manager-confirmed activity requires reason, preserves original and exposes De to Para',async({page})=>{
  ...
  await page.locator('#moveDate').selectOption('2026-09-23');await page.locator('#moveTime').fill('14:00');await page.locator('#moveSessionSave').click();await expect(page.locator('#moveReason')).toBeVisible();await page.locator('#moveReason').fill('Preciso realizar depois do almoço.');await page.locator('#moveSessionSave').click();
