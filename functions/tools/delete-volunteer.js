...
async function main(){
  ...
  try{
    ...
    const [activities,sessions,history]=await Promise.all([
      ...
      applicationDoc.ref.collection('history').get()
    ]);
    ...
    console.log(`Eventos de histórico: ${history.size}`);
    if(isCouple)console.log('\nATENÇÃO: esta candidatura é compartilhada. Os dois acessos serão excluídos.');

    console.log('\nSerão removidos: Authentication, users, volunteer_profiles, application, histórico, activities e activity_sessions.');
    ...
    console.log(`  Eventos de histórico removidos: ${history.size}`);
  }finally{rl.close()}
}
