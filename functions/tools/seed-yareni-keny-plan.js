...
async function main() {
  ...
  try {
    ...
    for (const row of planned) {
      ...
      const occupied = bySlot.get(sessionKey(row.date, row.time)) || [];
      if (occupied.length) {
        conflicts.push({
          planned: row,
          reason: `Horário já ocupado por ${occupied.map(item => item.activityName || item.activityId || item.id).join(', ')}.`,
        });
        continue;
      }
    ...
    console.log(`Administrador responsável: ${admin.email}`);
    console.log(`Grupo de todas as ocorrências: Livre`);
    console.log(`Status que será gravado: ${finalStatus}\n`);

    console.log('DISTRIBUIÇÃO POR DIA');
    usedActivities.forEach((activity, index) => {
      console.log(`${index + 1}. ${dates[index]} — ${activity.name}`);
      console.log(`   ${activity.slots.map(slot => `${slot.time} (${slot.period}, ${activity.duration} min)`).join(' | ')}`);
    });

    if (omittedActivities.length) {
      console.log('\nATIVIDADES QUE NÃO CABEM NO PERÍODO');
      omittedActivities.forEach((activity, index) => console.log(`  - ${usedActivities.length + index + 1}. ${activity.name}`));
    }

    console.log(`\nOcorrências planejadas: ${planned.length}`);
    console.log(`Já cadastradas exatamente iguais: ${alreadyPresent.length}`);
    console.log(`Novas ocorrências a criar: ${toCreate.length}`);
    console.log(`Conflitos: ${conflicts.length}`);

    if (alreadyPresent.length) {
      console.log('\nJÁ EXISTEM — SERÃO IGNORADAS');
      alreadyPresent.forEach(({ planned: row, existing: current }) => {
        console.log(`  ${row.date} ${row.time} — ${row.name} (sessão ${current.id})`);
      });
    }

    if (conflicts.length) {
      console.log('\nCONFLITOS — NENHUM DADO SERÁ ALTERADO');
      conflicts.forEach(({ planned: row, reason }) => console.log(`  ${row.date} ${row.time} — ${row.name}: ${reason}`));
      throw new Error('Há conflitos no planejamento. Resolva-os no Admin antes de executar novamente.');
    }

    if (!toCreate.length) {
      console.log('\n✓ Todas as ocorrências previstas já estão cadastradas exatamente como esperado. Nada a fazer.');
      return;
    }
    ...
    toCreate.forEach(row => {
      ...
      batch.set(sessionRef, {
        ...
        groupId: 'Livre',
        createdByUid: admin.uid,
        ...(finalStatus === 'confirmed' ? { confirmedAt: now } : {}),
        createdAt: now,
        updatedAt: now,
      });

      created.push({ activityId: activityRef.id, sessionId: sessionRef.id, ...row });
    });

    batch.update(appDoc.ref, {
      sessionCount: FieldValue.increment(created.length),
      activityCount: FieldValue.increment(created.length),
    ...
    console.log(`\n✓ ${created.length} ocorrência(s) criada(s) com sucesso.`);
    console.log(`✓ Grupo: Livre.`);
    console.log(`✓ application.sessionCount e application.activityCount incrementados em ${created.length}.`);
    console.log('\nCRIADOS');
    created.forEach(row => console.log(`  ${row.date} ${row.time} — ${row.name} (${row.duration} min)`));
    console.log('\nRode a auditoria para confirmar a integridade:');
    console.log(`  npm run admin:audit -- "${targetEmail}"`);
  } finally {
    rl.close();
  }
}
...
main().catch(error => {
  ...
  if (/credential|default credentials|Could not load/i.test(String(error?.message || ''))) {
    console.error('Se necessário no Cloud Shell: gcloud auth application-default login');
  }
  process.exitCode = 1;
});
